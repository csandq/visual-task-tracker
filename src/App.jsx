const onStepClick = (taskId, stepIdx) => {
  const task = tasks.find(t => t.id === taskId);
  if (!task) return;
  
  setStepModal({ taskId, stepIdx });
  
  if (stepIdx !== -1 && task.steps[stepIdx]) {
    // Copy step data to temporary state
    const stepData = {...task.steps[stepIdx]};
    setTempStep(stepData);
  } else {
    // Reset temp step for new step creation
    setTempStep({
      label: "",
      status: "todo",
      note: "",
...
// Temporary state objects for modals to avoid direct state manipulation
          ...
          <div className="bg-white p-6 rounded shadow w-full max-w-md">
            ...
            <div className="mb-3">
              ...
              <input 
                ...
                value={isNewStep ? newStep : tempStep.label} 
                onChange={(e) => isNewStep 
                  ? setNewStep(e.target.value) 
                  : setTempStep({...tempStep, label: e.target.value})
                } 
                onKeyDown={(e) => {
              ...
              <>
                <div className="mb-3">
                  ...
                  <textarea 
                    className="w-full border p-2 rounded" 
                    value={tempStep.note} 
                    onChange={(e) => setTempStep({...tempStep, note: e.target.value})} 
                  />
                </div>
                ...
                <div className="mb-3">
                  ...
                  <input 
                    type="date" 
                    className="w-full border p-2 rounded" 
                    value={tempStep.deadline || ""} 
                    onChange={(e) => setTempStep({...tempStep, deadline: e.target.value})} 
                  />
                </div>
                ...
                <div className="mb-4">
                  ...
                  <select 
                    className="w-full border p-2 rounded" 
                    value={tempStep.status} 
                    onChange={(e) => setTempStep({...tempStep, status: e.target.value})}
                  >
                    <option value="todo">Not Started</option>
                    <option value="in-progress">In Progress</option>
            ...
            <div className="flex justify-between mt-4">
              {isNewStep ? (
                ...
                <>
                  ...
                  <button 
                    className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600" 
                    onClick={() => updateStep(stepModal.taskId, stepModal.stepIdx, tempStep)}
                  >
                    Save
                  </button>
                </>

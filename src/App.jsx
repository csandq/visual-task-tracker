...
const onDragEnd = (result) => {
  ...
  const updatedSteps = Array.from(task.steps);
  
  // Remove the dragged item from its position
  const [reorderedItem] = updatedSteps.splice(source.index, 1);
  
  // Insert it in the new position
  updatedSteps.splice(destination.index, 0, reorderedItem);
  
  // Update the task with the new steps order
  setTasks(
    tasks.map(t => t.id === taskId ? { ...t, steps: updatedSteps } : t)
  );
        ...
        tasks.map(task => (
          <div key={task.id} className={`mb-6 p-3 border ${darkMode ? 'border-gray-700 bg-gray-700' : 'border-gray-200 bg-white'} rounded hover:shadow-md transition-colors duration-200`}>
            ...
            <div className="flex items-center space-x-6 overflow-x-auto py-2">
              <Droppable droppableId={`task-${task.id}`} direction="horizontal">
                {(provided) => (
                  <div 
                  ...
                  >
                    {task.steps.map((step, index) => (
                      <Draggable 
                        key={index} 
                        draggableId={`${task.id}-step-${index}`} 
                        index={index}
                      >
                        {(provided) => (
                          <div

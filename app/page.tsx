    ...
    <main className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <section className="workspace">
        ...
        {currentView === "settings" ? <section className="settings-page">
          ...
          <div className="view-title"><div><p className="eyebrow">DEADLINES</p><h2>{calendarLabel}</h2><p>Milestones from every active workstream, organized by deadline.</p></div><div className="calendar-actions"><div className="calendar-workspace-filter"><span>Workspace</span><Dropdown label="Calendar workspace" value={calendarWorkspace} onChange={setCalendarWorkspace} items={[{ value: "All workspaces", label: "All workspaces" }, ...Object.entries(workspaces).filter(([, info]) => info.active).map(([name]) => ({ value: name, label: name }))]} /></div><button aria-label="Previous month" onClick={() => setCalendarMonth((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))}>‹</button><button className="today" onClick={() => { const today = new Date(); setCalendarMonth(new Date(today.getFullYear(), today.getMonth(), 1)); }}>Today</button><button aria-label="Next month" onClick={() => setCalendarMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))}>›</button></div></div>
          ...
          <div className="view-title"><div><p className="eyebrow">YOUR NEXT ACTIONS</p><h2>My steps <span className="heading-count">{tasks.flatMap((task) => task.steps).filter((step) => step.status !== "done").length}</span></h2><p>Individual steps assigned to you across every workstream.</p></div><div className="view-title-actions"><label className="sort-control"><span>Sort by</span><Dropdown label="Sort steps" value={stepSortBy} onChange={(value) => setStepSortBy(value as typeof stepSortBy)} items={[{ value: "urgency", label: "Urgency" }, { value: "deadline", label: "Deadline" }]} /><button className="sort-direction" onClick={() => setStepSortDirection((value) => value === "asc" ? "desc" : "asc")} aria-label={`Sort ${stepSortDirection === "asc" ? "descending" : "ascending"}`}>{stepSortDirection === "asc" ? "↑" : "↓"}</button></label></div></div>
      ...
      {taskToEdit && (
        <aside className={`detail-panel task-editor ${detailsFullscreen ? "details-fullscreen" : ""}`}>
          ...
          <div className="panel-body">
            ...
            <div className="two-cols"><div><label className="field-label" htmlFor="task-workspace">Workspace</label><select id="task-workspace" className="recessed-select" value={taskToEdit.project} onChange={(e) => updateTask(taskToEdit.id, { project: e.target.value })}>{taskToEdit.backlog && <option value="Backlog">Backlog</option>}{Object.entries(workspaces).filter(([name, info]) => info.active || taskToEdit.project === name).map(([name]) => <option key={name}>{name}</option>)}</select></div><div><p className="field-label">Priority</p><div className="priority-pills" role="radiogroup" aria-label="Priority">{(["High", "Medium", "Low"] as Priority[]).map((priority) => <button type="button" key={priority} role="radio" aria-checked={taskToEdit.priority === priority} className={`priority-pill ${priority.toLowerCase()} ${taskToEdit.priority === priority ? "selected" : ""}`} onClick={() => updateTask(taskToEdit.id, { priority })}>{priority}</button>)}</div></div></div>
